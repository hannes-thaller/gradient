...
import org.sourceflow.gradient.monitoring.service.IntrospectService
import org.sourceflow.gradient.monitoring.service.MonitoringService
...
object DIContainer {
    ...
    val mongoURL: String by lazy {
        if (inContainer) {
            "mongodb://gs-monitoring-database:27017"
        } else {
            "mongodb://localhost:14002"
        }
    }
    ...
    val monitoringDao: MonitoringDao by lazy {
        MonitoringDao(mongoClient)
    }

    val monitoringService: MonitoringService by lazy {
        MonitoringService(pulsarClient, monitoringDao)
                .also { cleanRoutines.add(2 to it::close) }
    }

    internal val introspectService: IntrospectService by lazy {
        IntrospectService(pulsarClient, monitoringDao)
                .also { cleanRoutines.add(2 to it::close) }
    }

    const val grpcPort: Int = 14001
