...
from gradient.domain.services import inference
from gradient.domain.entities import dataset
...
class MessageService(inference.ModelServiceServicer):
    ...
    def __attrs_post_init__(self):
        ...
        consumer_code = self._client.subscribe("code", "gs-domain-service",
                                               message_listener=self.listen_for_code_messages)
        consumer_dataset = self._client.subscribe("dataset", "gs-domain-service",
                                                  message_listener=self.listen_for_dataset_messages)
        consumer_model = self._client.subscribe("domain", "gs-domain-service",
        ...
        object.__setattr__(self, "_producer_model", self._client.create_producer("domain"))

    def listen_for_dataset_messages(self, consumer: "pulsar.Consumer", msg: "pulsar.Message"):
        msg_dataset = dataset.DatasetMessage()
    ...
    def listen_for_model_messages(self, consumer: "pulsar.Consumer", msg: "pulsar.Message"):
        _logger.debug(f"Acknowledging irrelevant domain message.")
    ...
    def handle_dataset_handle_detail(self, msg: "dataset.DatasetMessage"):
        assert (msg.HasField("dataset_handle_detail"))
        # TODO issue domain training
        # TODO add dataset handle to queue, retrive dataset, fit domain, store domain, emit domain handle, update code elements with "hasModel"
