...
_dtype_to_data_type = {
    np.dtype(np.int32): entity.DataTypeDescriptor.INTEGER,
    np.dtype(np.int64): entity.DataTypeDescriptor.LONG,
    np.dtype(np.bool): entity.DataTypeDescriptor.BOOLEAN,
    np.dtype(np.float32): entity.DataTypeDescriptor.FLOAT,
    np.dtype(np.float64): entity.DataTypeDescriptor.DOUBLE,
    np.dtype(np.object): entity.DataTypeDescriptor.STRING,
}


class Transformation:
    def fit(self, arr: np.ma.masked_array, feature: entity.Feature) -> entity.Feature:
...
class ScaleTransformation(Transformation):
    ...
    def fit(self, arr: np.ma.masked_array, feature: entity.Feature) -> entity.Feature:
        ...
        data_type = entity.DataType(entity.DataTypeDescriptor.FLOAT)
...
class OrdinalTransformer(Transformation):
    ...
    def fit(self, arr: np.ma.masked_array, feature: entity.Feature) -> entity.Feature:
        ...
        return attr.evolve(feature, data_type=entity.DataType(_dtype_to_data_type[self.dtype_output]))
...
class BaseNTransformation(Transformation):
    ...
    def fit(self, arr: np.ma.masked_array, feature: entity.Feature) -> entity.Feature:
        ...
        data_type = entity.DataType(entity.DataTypeDescriptor.INTEGER)
...
@attr.s(slots=True)
class Pipeline:
    _transformation_factories: typing.List[typing.Callable[[entity.Feature, typing.List[Transformation]], Transformation]] = attr.ib()
    _features: typing.List[typing.List[entity.Feature]] = attr.ib(factory=list, init=False)
    _transformations: typing.List[typing.List[Transformation]] = attr.ib(factory=list, init=False)
    feature_description_input: entity.FeatureDescription = attr.ib(init=False)
    feature_description_output: entity.FeatureDescription = attr.ib(init=False)
    ...
    def fit(self, arrs: typing.List[np.array],
            feature_description: entity.FeatureDescription) -> entity.FeatureDescription:
        assert arrs
        ...
        self.feature_description_output = entity.FeatureDescription(feature_description.element_id, features)
    ...
    def transform_feature(self, arr: np.ndarray, feature: entity.Feature) -> np.ma.masked_array:
    ...
    def inverse_feature(self, arr: np.ndarray, feature: entity.Feature) -> np.ma.masked_array:
